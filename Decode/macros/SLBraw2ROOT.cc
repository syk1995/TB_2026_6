...
bool SLBraw2ROOT::ReadFile(TString inputFileName, bool overwrite, TString outFileName , bool zerosupression, bool getbadbcid_bool) {
    ...
    while(fin.is_open()) {
      ...
      if(_debug2) std::cout<<"dataResult hex:"<<hex<<dataResult<<"  dec:"<<dec<<dataResult<<std::endl;
      ...
      if(_eudaq) {
        ...
        temp_char=static_cast<int>(temp_char);
	      if(_debug2) std::cout<<"Read hex:"<<hex<<temp_char<<"       dec:"<<dec<<temp_char<<std::endl;
